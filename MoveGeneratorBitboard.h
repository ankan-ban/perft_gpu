...
__global__ void perft_bb_gpu_depth3(HexaBitBoardPosition **positions, CMove *moves, uint64 *globalPerftCounter, int nThreads)
{
    ...
    if (threadIdx.x == 0)
    {
        ...
        HexaBitBoardPosition **boardPointersSecondLevel;
        deviceMalloc(&boardPointersSecondLevel, sizeof(void *) * numSecondLevelMoves);


        // 4. now run interval expand

        // this call is a bit tricky :
        // we want the pointers of first level child boards to get replicated in the boardPointersSecondLevel array
        ...
        mgpu::IntervalExpandDGenValues(numSecondLevelMoves, secondLevelMoveCounts, (int) firstLevelChildBoards,     // WARNING: typecasting pointer to integer
                                       sizeof(HexaBitBoardPosition), numFirstLevelMoves, (int *) boardPointersSecondLevel, 
        ...
        makeMove_and_perft_single_level<<<nBlocks, BLOCK_SIZE, 0, childStream>>> (boardPointersSecondLevel, secondLevelChildMoves, globalPerftCounter, numSecondLevelMoves);
