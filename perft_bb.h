...
__global__ void perft_bb_gpu_depth3_hash(HexaBitBoardPosition **positions, uint64 *hashes, CMove *moves, uint64 **perftCounters, int nThreads)
{
    ...
    if (index < nThreads)
    {
        ...
        lookupHash = hash ^ (ZOB_KEY(depth) * 3);
        color = !color;
        uint64 entry = gShallowTT[lookupHash & (SHALLOW_TT_INDEX_BITS)];
        if ((entry & SHALLOW_TT_HASH_BITS) == (lookupHash & SHALLOW_TT_HASH_BITS))
        {
            // hash hit
    ...
    if (nMoves)
    {
        ...
        gShallowTT[lookupHash & (SHALLOW_TT_INDEX_BITS)] = (lookupHash & SHALLOW_TT_HASH_BITS)  |
